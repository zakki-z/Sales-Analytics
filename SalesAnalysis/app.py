...
def generate_distribution_plot(sales_data):
    ...
    ax.set_title('Sales Categories Distribution', fontsize=16)
    ax.set_ylabel('Number of Products', fontsize=12)
    ax.set_xlabel('Sales Category', fontsize=12)
...
def generate_prediction_plot(new_products):
    ...
    ax.set_title('New Sales Trend Predictions', fontsize=16)
    ax.set_xlabel('Sales (Simulated)', fontsize=12)
    ax.set_ylabel('Growth Trend', fontsize=12)
    ax.legend(title='Best Seller (1=Yes, 0=No)', loc='upper left')
    return create_figure_as_base64(fig)
...
def index():
    ...
    if sales_data is None:
        return render_template('error.html',
                               message="CSV file not found. Please make sure 'restaurant_sales.csv' is available.")
